...
async def create_dish(menu_id: str, submenu_id: str, dish_data: DishCreate, response: DishService = Depends()):
    return await response.create_dish(menu_id, submenu_id, dish_data)


@router.get('/dishes', response_model=list[DishResponse])
async def read_dishes(menu_id: str, submenu_id: str, response: DishService = Depends()):
    return await response.read_dishes(menu_id, submenu_id)


@router.get('/dishes/{dish_id}', response_model=DishResponse)
async def read_dish(menu_id: str, submenu_id: str, dish_id: str, response: DishService = Depends()):
...
async def delete_dish(menu_id: str, submenu_id: str, dish_id: str, response: DishService = Depends()):
