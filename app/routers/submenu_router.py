...
async def create_submenu(menu_id: str, submenu_data: SubMenuCreate, response: SubMenuService = Depends()) -> SubMenuResponse:
    return await response.create_submenu(menu_id, submenu_data)


@router.get('/submenus', response_model=list[SubMenuResponse])
async def read_submenus(menu_id: str, response: SubMenuService = Depends()) -> list[SubMenuResponse]:
    return await response.read_submenus(menu_id)


@router.get('/submenus/{submenu_id}', response_model=SubMenuResponse)
async def read_submenu(menu_id: str, submenu_id: str, response: SubMenuService = Depends()) -> SubMenuResponse:
...
async def delete_submenu(menu_id: str, submenu_id: str, response: SubMenuService = Depends()) -> dict:
